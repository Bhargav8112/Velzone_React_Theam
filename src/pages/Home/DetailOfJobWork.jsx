    ...
    const  JobWorkPassData = [
        {
          ...
          const handleEdit = (item) => {
            const updatedValue = prompt("Edit Raw Material Qty:", item.Raw_Material_Qty);
            if (updatedValue !== null) {
                const updatedData = jobWorkData.map((data) =>
                    data.sr_no === item.sr_no ? { ...data, Raw_Material_Qty: updatedValue } : data
                );
                setJobWorkData(updatedData);
                localStorage.setItem("jobWorkData", JSON.stringify(updatedData));
            }
        };
    
        // Function to handle deleting an entry
        const handleDelete = (sr_no) => {
            const updatedData = jobWorkData.filter((item) => item.sr_no !== sr_no);
            setJobWorkData(updatedData);
            localStorage.setItem("jobWorkData", JSON.stringify(updatedData));
        };
  return (
    <div className='page-content'>
                ...
                <div className="col-lg-12">
                    <div className="card">
                        <div className="card-header align-items-center d-flex">
                            <h4 className="card-title mb-0 flex-grow-1">Jobwork Gate Pass</h4>
                        </div>
      ...
      <DetailsOfJobWork  data={jobWorkData} onEdit={handleEdit} onDelete={handleDelete}  />
            </div>
    </div>
  )
}
